...
| 10 | **IN_COLLECTION** | object → node | 🟢 live | managed,nwextract | · | Object → CONTAINER(Collection). — *Authored layer/collection-tree membership. THE single-valued scene-tree axis: one edge per object → its innermost container, ancestors nest via CONTAINER def_ref. Managed connectors emit layer/collection trees; nwextract emits the authored Navis selection tree — structural levels (file/layer/collection) always mint, single-object instance wrappers collapse [ENG-9218].* |
...
| 6 | **COLLECTION** | ⚪ retired | · | · | Authored layer/collection node. — *Retired in v5: folded into CONTAINER (subtype=Collection). Stays retired post-ENG-9218: the revived authored-tree concept ships as CONTAINER rows, not as this kind.* |
...
| subtype | VARCHAR | CONTAINER polymorphism: Collection \| Model \| MEP System \| Network \| Group. The single grouping discriminator (replaced the former units-overload). |
