...
-- ── sgeo catalogs (spec-side vocabulary; NOT shipped in bundles) ─────────────
--  SGEO is the binary geometry blob format inside geometries.content (16-byte
--  header + per-primitive body; full layout in docs/rationale/sgeo-format.md).
--  These two catalogs are the SOT for its enums, extracted from the reference
--  implementation (speckle-sharp-sdk SgeoFormat.cs) — they are NOT bundle_files
--  members: SGEO headers are self-describing bytes, so nothing ships. Eight
--  hand-synced implementations must match these rows (see the rationale doc's
--  census + audit); codegen for sgeo constants is future work.
