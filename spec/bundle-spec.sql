...
CREATE TABLE rel_types (
  ...
  emitted_by    VARCHAR,      -- csv of producers: rvextract | nwextract | dwgextract | dgnextract | skpextract | teklaextract | managed | archicad
...
INSERT INTO rel_types
  ...
  (7,  'ON_LEVEL',         'object',          'node',     'live',     'rvextract,nwextract,managed,archicad', NULL, 'Object → LEVEL node.',                            'Storey membership; also the default scene-view tier.'),
  ...
  (10, 'IN_COLLECTION',    'object',          'node',     'live',     'nwextract,dwgextract,dgnextract,skpextract,managed,archicad', NULL, 'Object → CONTAINER(Collection).',                 'Authored layer/collection-tree membership.'),
  ...
  (12, 'IN_ROOM',          'object',          'object',   'live',     'rvextract,managed,archicad', NULL,      'Object → containing room object.',                'Spatial occupancy (furniture/door/window → room). Rooms are objects, not nodes.'),
  ...
  (17, 'IN_GROUP',         'object',          'node',     'live',     'rvextract,dwgextract,managed,archicad', NULL, 'Object → CONTAINER(Group).',                      'Authored scene-group membership (Rhino/AutoCAD groups). A separate axis from IN_COLLECTION: an object keeps its layer AND its group(s); groups may nest (CONTAINER def_ref) and overlap. Un-retired post-v5.'),
  ...
  (22, 'HOSTED_ON',        'object',          'object',   'live',     'rvextract,teklaextract,managed,archicad', NULL, 'Hosted element → host.',                          'Revit hosting (door/window → wall, fixture → ceiling/floor/face) from ODA getHostId. A DIFFERENT semantic from SUBELEMENT ownership (owningElemId): a door is placed on a wall, not a component of it. Emitted only when the element has no owner (legacy precedence) and both endpoints are converted. Un-retired post-v5.'),
  (23, 'BOUNDS',           'object',          'object',   'live',     'rvextract,archicad',  NULL,      'Bounding wall → room object.',                    'Room footprint (which walls bound a room) for downstream egress / plan analysis.'),
